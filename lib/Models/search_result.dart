/// Student Note:
/// I use `SearchResult` to represent a single item returned by TMDB's
/// multi-search endpoint. A single `SearchResult` can represent a movie, a
/// TV show, or a person depending on the `media_type` field.
///
/// Fields are nullable because the API sometimes omits keys depending on the result type.
class SearchResult {
  SearchResult({
    bool? adult,
    String? backdropPath,
  ...
  /// Student Note:
  /// Construct a model instance from raw JSON returned by TMDB.
  /// The list is heterogeneous (movies/tv/people), so some keys (like title vs name) vary.
  SearchResult.fromJson(dynamic json) {
    _adult = json['adult'];
    _backdropPath = json['backdrop_path'];
    // genre_ids can be absent for some results (e.g., person), default to []
