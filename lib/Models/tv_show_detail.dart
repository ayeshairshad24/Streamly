...
/// Student Note:
/// `TvShowDetail` represents the detailed payload for a TV show from TMDB's
/// /tv/{id} endpoint. I use this to render the TV show detail screen —
/// episodes, seasons, runtime per episode, and next/last episode information.
///
/// Important fields: `name`, `overview`, `posterPath`, `seasons`,
/// `episodeRunTime`, `nextEpisodeToAir`, `lastEpisodeToAir`, `numberOfSeasons`.
///
/// The model contains several nested lists (created_by, networks, seasons).
/// These reflect the API structure and are directly displayed by the UI.
class TvShowDetail {
  TvShowDetail({
    bool? adult,
    String? backdropPath,
  ...
  /// Student Note:
  /// Parse JSON into `TvShowDetail`.
  /// Arrays are converted to typed lists where present; nested objects are parsed by their own `.fromJson` constructors.
...
/// Student Note:
/// `Seasons` describes a single season in a TV show.
class Seasons {
  Seasons({
    String? airDate,
    num? episodeCount,
...
/// Student Note:
/// `ProductionCountries` maps the country's iso code to its name.
...
/// Student Note:
/// `ProductionCompanies` provides optional `logoPath` and `name` for display.
...
/// Student Note:
/// `Networks` lists broadcasting networks for the series.
class Networks {
  Networks({
    num? id,
    String? name,
...
/// Student Note:
/// `NextEpisodeToAir` contains metadata about the upcoming scheduled episode.
...
/// Student Note:
/// `LastEpisodeToAir` contains metadata about the most recently aired episode.
...
/// id : 89365
/// credit_id : "593885c392514101d200676a"
/// name : "Craig Mazin"
/// gender : 2
/// profile_path : "/uETz82u05kR1qZq2t7F0d1tJ2V2.jpg"

/// Student Note:
/// `CreatedBy` lists the creators/authors of the TV show.
class CreatedBy {
  CreatedBy({
    num? id,
    String? creditId,
